...
import {
  ...
  TrendingUp, Zap, FlaskConical, Calculator, Globe,
  Microscope, Cpu, ChevronRight, ArrowRight, CheckCircle, MapPin,
  Star,
} from "lucide-react";
...
const TUTORS = [
  {
    id: 1, name: "Mahbubur Rahman", initials: "MR",
    university: "BUET — EEE", location: "Dhaka",
    subjects: ["Higher Math", "Physics", "ICT"],
    rating: 5.0, reviews: 148, rate: 450,
    badge: "Top Rated",
  ...
  {
    id: 2, name: "Nasrin Sultana", initials: "NS",
    university: "DU — Chemistry", location: "Chittagong",
    subjects: ["Chemistry", "Biology", "HSC Prep"],
    rating: 4.9, reviews: 96, rate: 380,
    badge: "Verified",
  ...
  {
    id: 3, name: "Saiful Alam", initials: "SA",
    university: "Rajshahi University", location: "Rajshahi",
    subjects: ["English", "Bangla", "SSC/HSC"],
    rating: 4.7, reviews: 203, rate: 320,
    badge: "500+ Students",
...
const CATEGORIES = [
  { label: "Class 6–8",       icon: BookOpen,      count: "120+ tutors", color: "text-teal-600",   bg: "bg-teal-50"   },
  { label: "Class 9–10",      icon: GraduationCap, count: "210+ tutors", color: "text-blue-600",   bg: "bg-blue-50"   },
  { label: "SSC Exam",        icon: Award,         count: "180+ tutors", color: "text-orange-600", bg: "bg-orange-50" },
  { label: "HSC Exam",        icon: TrendingUp,    count: "160+ tutors", color: "text-rose-600",   bg: "bg-rose-50"   },
  { label: "Admission Test",  icon: Zap,           count: "90+ tutors",  color: "text-purple-600", bg: "bg-purple-50" },
];

const SUBJECTS = [
  { name: "Mathematics", icon: Calculator,    tutors: 185 },
  { name: "Chemistry",   icon: FlaskConical,  tutors: 120 },
  { name: "Physics",     icon: Zap,           tutors: 135 },
  { name: "Biology",     icon: Microscope,    tutors: 98  },
  { name: "English",     icon: Globe,         tutors: 160 },
  { name: "ICT",         icon: Cpu,           tutors: 85  },
];

const REVIEWS = [
  { name: "Rahela Parvin",  cls: "SSC Candidate · Dhaka",     initials: "RP", rating: 5, text: "Got an A+ in Math after studying with sir. His way of explaining concepts is incredible — I used to dread math before." },
  { name: "Arif Hossain",   cls: "HSC Candidate · Chittagong",initials: "AH", rating: 5, text: "Studied Chemistry with Nasrin madam and scored really well in HSC. Online sessions were smooth and well-structured." },
  { name: "Mitu Akter",     cls: "Class 9 · Rajshahi",        initials: "MA", rating: 5, text: "Booking a tutor on SkillBridge is super easy. Found my perfect match in under 10 minutes. Amazing platform!" },
];

const STEPS = [
  { num: "01", title: "Pick a Subject",  desc: "Choose the subject or class level you need help with." },
  { num: "02", title: "Browse Tutors",   desc: "Filter by rating, price and experience to find your match." },
  { num: "03", title: "Book a Slot",     desc: "Pick a date and time that works best for your schedule." },
  { num: "04", title: "Start Learning",  desc: "Study online or in-person — completely your choice." },
];
...
function HeroBanner() {
  return (
    <div className="relative w-full max-w-[540px] mx-auto">
      {/* soft background */}
      <div className="absolute inset-0 rounded-3xl bg-gradient-to-br from-emerald-50 via-teal-50 to-sky-50" />
      ...
      >
        ...
        <ellipse cx="260" cy="430" rx="185" ry="26" fill="#bbf7d0" opacity="0.45" />
    ...
    <div className="min-h-screen bg-[#faf9f7] text-zinc-900">
      ...
      <section className="max-w-7xl mx-auto px-6 pt-20 pb-24 grid lg:grid-cols-2 gap-16 items-center">
        ...
        <div>
          ...
          <div className="flex gap-8 pt-6 border-t border-zinc-100">
            {[["5,800+","Active Students"],["640+","Verified Tutors"],["98%","Satisfaction Rate"]].map(([n,l]) => (
              <div key={l}>
                <div className="text-2xl font-extrabold text-emerald-600 tracking-tight">{n}</div>
                <div className="text-xs text-zinc-400 mt-0.5 font-medium">{l}</div>
              </div>
            ))}
          </div>
        </div>
      ...
      <section className="bg-white border-y border-zinc-100 py-16">
        <div className="max-w-7xl mx-auto px-6">
          <p className="text-xs font-bold tracking-widest text-emerald-600 uppercase mb-2">Browse by Level</p>
          <h2 className="text-3xl font-extrabold tracking-tight mb-10">Which class do you need help with?</h2>
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-4">
            {CATEGORIES.map(({ label, icon: Icon, count, color, bg }) => (
              <Card key={label}
                className="group cursor-pointer border border-zinc-100 hover:border-emerald-300 hover:shadow-md transition-all duration-200 hover:-translate-y-1">
                <CardContent className="p-5 text-center">
                  <div className={`w-11 h-11 rounded-xl ${bg} group-hover:bg-emerald-50 flex items-center justify-center mx-auto mb-3 transition-colors`}>
      ...
      <section className="py-20">
        <div className="max-w-7xl mx-auto px-6">
          <div className="flex items-end justify-between mb-10">
            <div>
              <p className="text-xs font-bold tracking-widest text-emerald-600 uppercase mb-2">Featured</p>
              <h2 className="text-3xl font-extrabold tracking-tight">Meet our top tutors</h2>
            </div>
            <Button variant="ghost" className="text-emerald-600 font-semibold text-sm group">
              View all tutors <ArrowRight size={14} className="ml-1.5 group-hover:translate-x-1 transition-transform" />
            </Button>
          </div>

          <div className="grid md:grid-cols-3 gap-5">
            {TUTORS.map(t => (
              <Card key={t.id} className="group border border-zinc-100 hover:shadow-lg transition-all duration-200 hover:-translate-y-1">
                <CardHeader className="p-5 pb-4">
                  <div className="flex gap-3 items-start">
                    <Avatar className={`h-12 w-12 rounded-xl shrink-0 ${t.avatarClass}`}>
                      <AvatarFallback className={`font-bold text-base rounded-xl ${t.avatarClass}`}>{t.initials}</AvatarFallback>
                    </Avatar>
                    <div className="flex-1 min-w-0">
                      <div className="font-bold text-base leading-tight">{t.name}</div>
                      <div className="text-xs text-zinc-400 mt-0.5 flex items-center gap-1"><GraduationCap size={11}/> {t.university}</div>
                      <div className="text-xs text-zinc-400 mt-0.5 flex items-center gap-1"><MapPin size={11}/> {t.location}</div>
                      <Badge variant="outline" className={`mt-1.5 text-[10px] font-semibold ${t.badgeClass}`}>{t.badge}</Badge>
                    </div>
                  </div>
                </CardHeader>
                <CardContent className="px-5 pb-4">
                  <div className="flex flex-wrap gap-1.5 mb-4">
                    {t.subjects.map(s => (
                      <Badge key={s} variant="secondary" className="text-xs rounded-md bg-zinc-100 text-zinc-700 font-medium">{s}</Badge>
                    ))}
                  </div>
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-xs text-zinc-400">{t.rating} ({t.reviews})</span>
                    </div>
                    <div>
                      <span className="font-extrabold text-base">৳{t.rate}</span>
      ...
      <section className="py-20 bg-white border-y border-zinc-100">
        <div className="max-w-7xl mx-auto px-6">
          <div className="text-center mb-14">
            <p className="text-xs font-bold tracking-widest text-emerald-600 uppercase mb-2">How It Works</p>
            <h2 className="text-3xl font-extrabold tracking-tight">Get started in 4 simple steps</h2>
          </div>
      ...
      <section className="py-20 bg-emerald-700">
        <div className="max-w-7xl mx-auto px-6">
          <p className="text-xs font-bold tracking-widest text-emerald-300 uppercase mb-2">All Subjects</p>
          <h2 className="text-3xl font-extrabold text-white tracking-tight mb-10">Tutors available for every subject</h2>
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-3">
            {SUBJECTS.map(({ name, icon: Icon, tutors }) => (
              <div key={name}
                ...
                <div className="flex-1">
                  ...
                  <div className="text-xs text-emerald-300/60 mt-0.5">{tutors} tutors available</div>
                </div>
                <ChevronRight size={16} className="text-white/20 group-hover:text-emerald-300 group-hover:translate-x-1 transition-all" />
              </div>
            ))}
          </div>
        </div>
      ...
      <section className="py-20 bg-[#faf9f7]">
        <div className="max-w-7xl mx-auto px-6">
          <p className="text-xs font-bold tracking-widest text-emerald-600 uppercase mb-2">Reviews</p>
          <h2 className="text-3xl font-extrabold tracking-tight mb-10">What students are saying</h2>
          <div className="grid md:grid-cols-3 gap-5">
            {REVIEWS.map(({ name, cls, initials, text, rating }) => (
              <Card key={name} className="border border-zinc-100 hover:shadow-md transition-shadow">
                <CardContent className="p-6">
                  ...
                  <div className="flex items-center gap-3">
                    <Avatar className="h-9 w-9">
                      <AvatarFallback className="bg-emerald-50 text-emerald-700 font-bold text-xs">{initials}</AvatarFallback>
                    </Avatar>
                    <div>
      ...
      <section className="py-24 bg-zinc-900">
        <div className="max-w-3xl mx-auto px-6 text-center">
          ...
          <p className="text-zinc-400 text-lg mb-8">
            Sign up now and enjoy your first session with any tutor completely free of charge.
          </p>
          <div className="flex flex-col sm:flex-row gap-3 justify-center">
            <Button size="lg" className="bg-emerald-500 hover:bg-emerald-400 text-white rounded-xl px-8 font-semibold text-base">
      ...
      <footer className="bg-zinc-950 border-t border-zinc-800 py-10 px-6">
        <div className="max-w-7xl mx-auto flex flex-col sm:flex-row items-center justify-between gap-6">
          <div className="flex items-center gap-1.5">
            ...
            <span className="font-bold text-white">Skill<span className="text-emerald-500">Bridge</span></span>
          </div>
          <div className="flex gap-6 flex-wrap justify-center">
            {["Find Tutors","Become a Tutor","About Us","Contact","Privacy Policy"].map(l => (
              <a key={l} href="#" className="text-xs text-zinc-500 hover:text-white transition-colors">{l}</a>
            ))}
          </div>
          <div className="text-xs text-zinc-600">© 2025 SkillBridge. All rights reserved.</div>
        </div>
      </footer>
    </div>
  );
